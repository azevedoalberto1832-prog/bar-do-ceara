...
type TicketStatus = 'adicionado'|'preparo'|'pronto'|'entrega'|'concluido'|'cancelado'
...
const needsKitchen=(item:{name:string})=>/jantinha|almo[cç]o/i.test(item.name)
...
export default function App(){
 ...
 const sendKitchenTicket=(serviceTabId:number,items:TicketItem[])=>{
   ...
   const ticket:KitchenTicket={id:Math.max(0,...serviceTabs.flatMap(t=>t.tickets.map(x=>x.id)))+1,createdAt:new Date().toISOString(),status:normalized.some(needsKitchen)?'preparo':'adicionado',items:normalized,stockReserved:true}
 ...
 return <div className="app-shell">
   ...
   <main className="main">
    ...
    {tab==='cozinha'&&<Section title="Cozinha e retirada" subtitle="Pedidos em produção, prontos e entregues no balcão.">
      <div className="operation-metrics"><div><Clock3/><span>Em preparo</span><strong>{operationalTickets.filter(x=>x.ticket.status==='preparo'&&x.ticket.items.some(needsKitchen)).length}</strong></div><div><CheckCircle2/><span>Prontos</span><strong>{operationalTickets.filter(x=>x.ticket.status==='pronto'&&x.ticket.items.some(needsKitchen)).length}</strong></div><div><Bike/><span>Em entrega</span><strong>{operationalTickets.filter(x=>x.ticket.status==='entrega').length}</strong></div></div>
      <div className="kitchen-board">{operationalTickets.filter(x=>['preparo','pronto'].includes(x.ticket.status)&&x.ticket.items.some(needsKitchen)).map(x=><OperationTicketCard key={x.ticket.id} {...x} visibleItems={x.ticket.items.filter(needsKitchen)} onStatus={updateTicketStatus} onCancel={cancelTicket}/>)}</div>
      {!operationalTickets.some(x=>['preparo','pronto'].includes(x.ticket.status)&&x.ticket.items.some(needsKitchen))&&<div className="empty-table">Nenhuma Jantinha ou Almoço aguardando a cozinha.</div>}
    </Section>}
...
function ServiceTabModal({serviceTab,client,products,onClose,onSend,onCloseTab}:{serviceTab:ServiceTab;client?:Client;products:Product[];onClose:()=>void;onSend:(serviceTabId:number,items:TicketItem[])=>boolean;onCloseTab:()=>void}){
 ...
 return <div className="modal-backdrop"><div className="modal service-tab-modal"><button className="modal-x" onClick={onClose}><X/></button><div className="service-modal-heading"><div><span>Comanda #{String(serviceTab.id).padStart(3,'0')}</span><h2>{client?.name}</h2><p>{client?.phone} · {serviceTab.mode==='local'?'Consumo local':serviceTab.mode==='retirada'?'Retirada':'Delivery'}{serviceTab.table?` · Mesa ${serviceTab.table}`:''}</p></div><div><small>Total atual</small><strong>{money(total)}</strong><button className="close-tab-button" disabled={!allItems.length} onClick={()=>draft.length?alert('Adicione ou remova os itens do novo pedido antes de fechar a comanda.'):onCloseTab()}>Fechar comanda</button></div></div><div className="service-workspace"><section><div className="search compact"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar no cardápio"/></div><div className="service-product-list">{shown.map(p=><button key={p.id} onClick={()=>add(p)} disabled={tracked(p)&&p.stock<=0}><span><b>{p.name}</b><small>{p.category} · {needsKitchen(p)?'Vai para cozinha':'Vai direto à comanda'} · {tracked(p)?`${p.stock} disponíveis`:'Venda livre'}</small></span><strong>{money(p.price)}</strong></button>)}</div></section><aside><h3>Itens para adicionar</h3>{details.length?<div className="draft-lines">{details.map(i=><div key={i.productId}><span><b>{i.product.name}</b><small>{money(i.product.price*i.qty)}</small></span><div className="qty-control"><button onClick={()=>change(i.product,-1)}>−</button><b>{i.qty}</b><button onClick={()=>change(i.product,1)}>+</button></div></div>)}</div>:<p className="muted">Clique nos itens para montar o pedido.</p>}<label>Observação deste pedido<input value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Ex.: sem cebola, carne bem passada"/></label><div className="draft-total"><span>Total</span><strong>{money(draftTotal)}</strong></div><button className="primary" disabled={!draft.length} onClick={()=>{const sent=onSend(serviceTab.id,details.map(i=>({productId:i.product.id,name:i.product.name,qty:i.qty,price:i.product.price,trackStock:i.product.trackStock,notes:notes.trim()||undefined})));if(sent){setDraft([]);setNotes('')}}}>Adicionar à comanda</button><small className="kitchen-routing-note">Jantinha e Almoço vão para a cozinha. Os demais itens ficam somente na comanda.</small></aside></div>{serviceTab.tickets.length>0&&<div className="ticket-history"><h3>Itens adicionados</h3>{serviceTab.tickets.map(t=><div className={`ticket-row ticket-${t.status}`} key={t.id}><span><b>Pedido #{String(t.id).padStart(3,'0')}</b><small>{new Date(t.createdAt).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})} · {ticketStatusLabel(t.status)}</small></span><p>{t.items.map(i=>`${i.qty}x ${i.name}`).join(' · ')}</p><strong>{money(t.items.reduce((s,i)=>s+i.qty*i.price,0))}</strong></div>)}</div>}</div></div>
}

const ticketStatusLabel=(status:TicketStatus)=>({adicionado:'Na comanda',preparo:'Em preparo',pronto:'Pedido pronto',entrega:'Saiu para entrega',concluido:'Concluído',cancelado:'Cancelado'}[status])

function OperationTicketCard({serviceTab,ticket,client,visibleItems,onStatus,onCancel}:{serviceTab:ServiceTab;ticket:KitchenTicket;client?:Client;visibleItems?:TicketItem[];onStatus:(serviceTabId:number,ticketId:number,status:TicketStatus)=>void;onCancel:(serviceTabId:number,ticket:KitchenTicket)=>void}){
